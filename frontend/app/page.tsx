...
export default function Home() {
	...
	const [chartMode, setChartMode] = useState('game'); // 'game' ou 'day'
	...
	const processData = (data) => {
		...
		const mmrData = [];
		const dayMap = {}; // Pour stocker la dernière game de chaque jour
		...
		sortedData.forEach((row, index) => {
			...
			if (row['MMR After']) {
				...
				// On écrase la valeur pour ce jour : à la fin de la boucle, dayMap contiendra la DERNIÈRE game du jour
				if (gameDate) {
					dayMap[gameDate] = gameEntry;
				}
			}
		...
		// On transforme le dictionnaire des jours en tableau chronologique
	...
	// Tooltip customisé qui s'adapte au mode choisi
	...
	return (
		<main className="p-8 bg-gray-900 text-white min-h-screen font-sans">
			<div className="max-w-7xl mx-auto">
				...
				{globalStats && (
					<div className="mb-10 bg-gray-800 rounded-2xl shadow-2xl border border-gray-700 p-6 flex flex-col lg:flex-row gap-8">
						...
						{/* Partie gauche : Statistiques Globales */}
								...
								<div className="flex-grow w-full">
									<ResponsiveContainer width="100%" height="100%">
										<LineChart data={currentChartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
											...
											{/* On change l'axe X selon le mode choisi */}
											<XAxis
												dataKey={chartMode === 'game' ? 'match' : 'date'}
												stroke="#6b7280"
												tick={{fontSize: 12}}
												minTickGap={20}
											/>
				...
				{stats && (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
						{stats.map((archetype, idx) => {
							...
							return (
								<div key={idx} className="bg-gray-800 rounded-2xl shadow-xl overflow-hidden flex flex-col justify-between transform transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/50 border border-gray-700">

									<div className="h-3 w-full" style={{ background: `linear-gradient(to right, ${color1}, ${color2})` }}></div>
